    ...
    return (
        <div className="uk-container" onClick={() => setEditMode({status: false, rowId: 0})}>
            <div className="uk-flex uk-align-right uk-margin-top">
                <button className="uk-button-large uk-border-pill uk-button-secondary uk-text-uppercase uk-margin-right"
                        onClick={() => navigate(CALCULATE_ROUTE)}>
                    Calculate
                </button>
                <button className="uk-button-large uk-border-pill uk-button-primary uk-text-uppercase"
                        disabled={editableUsersId.length === 0} onClick={() => editedUsers()}>
                    Save
                </button>
            </div>
            <table class="uk-table uk-table-hover uk-table-divider uk-table-middle">
...

                {
                    users.map((item) => (
                        <tr key={item.id}>
                            <td>{item.id}</td>
                            <td className="uk-table-expand" onClick={(e) => {
                                e.stopPropagation();
                                setEditMode({status: true, rowId: item.id, field: "RegistrationDate"})
                            }}>
                                {
                                    editMode.status && editMode.rowId == item.id && editMode.field == "RegistrationDate"
                                        ...
                                        <form className="uk-width-1-2">
                                            <input className="uk-input" type="date"
                                                   value={item.registrationDate.substring(0, 10)} onChange={(e) => {
                                                setUsers(users.map(user => user.id === item.id ? {
                                                    ...user,
                            ...
                            <td className="uk-table-expand" onClick={(e) => {
                                e.stopPropagation();
                                setEditMode({status: true, rowId: item.id, field: "LastActivityDate"})
                            }}>
                                {
                                    editMode.status && editMode.rowId == item.id && editMode.field == "LastActivityDate"
                                        ...
                                        <form className="uk-width-1-2">
                                            <input className="uk-input" type="date"
                                                   value={item.lastActivityDate.substring(0, 10)} onChange={(e) => {
                                                setUsers(users.map(user => user.id === item.id ? {
                                                    ...user,
